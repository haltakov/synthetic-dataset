...
GRAPHICS_FALLBACK::GRAPHICS_FALLBACK(bool enable_flow_mode) :
	initialized(false),
	using_shaders(false),
	max_anisotropy(0),
	shadows(false),
...
void GRAPHICS_FALLBACK::DrawScene(std::ostream & error_output)
{
	...
	// Set the active matrix back to 1 for each drawable back to 0
	if (flow_mode) {
		for (std::vector <GRAPHICS_CONFIG_PASS>::const_iterator i = config.passes.begin(); i != config.passes.end(); i++)
		{
			if (i->conditions.Satisfied(conditions))
				...
				else
				{
					...
					for (std::vector <std::string>::const_iterator d = i->draw.begin(); d != i->draw.end(); d++)
					{
						...
						for (int cubeside = 0; cubeside < cubesides; cubeside++)
						{
							...
							// set AM back to 0
							std::vector <DRAWABLE*> & dynamic_drawlist = *container;
							std::vector <DRAWABLE*> & static_drawlist = container_static->second;
						
							for (auto ptr = dynamic_drawlist.begin(); ptr != dynamic_drawlist.end(); ++ptr) 
								(*ptr)->activeMVMatrix = 0;

							for (auto ptr = static_drawlist.begin(); ptr != static_drawlist.end(); ++ptr) 
								(*ptr)->activeMVMatrix = 0;
						}
					}
				}
