...
GAME::GAME(std::ostream & info_out, std::ostream & error_out) :
	...
	replay(timestep),
	http("/tmp"),
	start_camera_pose(false),
	sequence_id(0),
	should_record(false),
	dataset_render_mode(RENDER_MODE_CAMERA),
	frames_to_record(1),
	replay_file(""),
...
{
	pathmanager.Init(info_output, error_output, dataset_render_mode);
	...
	std::string car_parts_path = pathmanager.GetCarPartsPath();
	if (dataset_render_mode == RENDER_MODE_DEPTH ||
		dataset_render_mode == RENDER_MODE_FLOW_X ||
		dataset_render_mode == RENDER_MODE_FLOW_Y ||
		dataset_render_mode == RENDER_MODE_SEGMENTATION) {
		car_parts_path += "/segmentation/";
	}
	...
	content.addSharedPath(car_parts_path);
	...
	for (int i = 0; i < rendererCount; i++)
	{
		...
		if (false && enableGL3 && i == 0 && settings.GetShaders())
		{
		...
		{
			graphics_interface = new GRAPHICS_FALLBACK(dataset_render_mode == RENDER_MODE_FLOW_X || dataset_render_mode == RENDER_MODE_FLOW_Y);
...
bool GAME::ParseArguments(std::list <std::string> & args)
{
	...
	{
		pathmanager.Init(info_output, error_output, RENDER_MODE_CAMERA);
	...
	if (argmap.find("-carlist") != argmap.end()) {
		carlist_filename = argmap["-carlist"];
		cars_list = ReadCarList(carlist_filename);
	}

	if (argmap.find("-camerapose") != argmap.end()) {
		std::string t = argmap["-camerapose"];
		std::replace(t.begin(), t.end(), ',', ' ');
		std::stringstream camera_pose(t);
		camera_pose >> start_camera_pos[0] >> start_camera_pos[1] >> start_camera_pos[2];
		camera_pose >> start_camera_orient[0] >> start_camera_orient[1] >> start_camera_orient[2] >> start_camera_orient[3];
		start_camera_pose = true;
	}

	if (argmap.find("-seq") != argmap.end()) {
		sequence_id = atoi(argmap["-seq"].c_str()) - 1;
	}
	...
	if (argmap.find("-dataset_mode") != argmap.end()) {
		...
		int mode = atoi(argmap["-dataset_mode"].c_str());
		switch (mode) {
			case 0:  dataset_render_mode = RENDER_MODE_CAMERA;			
					 renderconfigfile = "render.conf.deferred";
					 break;

			case 1:  dataset_render_mode = RENDER_MODE_DEPTH;			
					 renderconfigfile = "render_depth.conf.deferred";
					 break;

			case 2:  dataset_render_mode = RENDER_MODE_FLOW_X;			
					 renderconfigfile = "render_flow_x.conf.deferred";
					 break;

			case 3:  dataset_render_mode = RENDER_MODE_FLOW_Y;		
					 renderconfigfile = "render_flow_y.conf.deferred";
					 break;

			case 4:  dataset_render_mode = RENDER_MODE_SEGMENTATION;			
					 renderconfigfile = "render.conf.deferred";
					 break;

			case 5:  dataset_render_mode = RENDER_MODE_POSE;			
					 renderconfigfile = "render.conf.deferred";
					 break;
		}
	...
	if (argmap.find("-frames_to_record") != argmap.end()) {
		frames_to_record = atoi(argmap["-frames_to_record"].c_str());
	}
...
void GAME::MainLoop()
{
	...
	while (!eventsystem.GetQuit() && (!benchmode || replay.GetPlaying()))
	{
		...
		if (replay_started && !replay.GetPlaying()) {
			replay_started = false;

			if (quit_after_replay)
				QuitGame();
			else
				LeaveGame();
		}
		...
		if (should_record && replay.GetPlaying() && !gui.Active() && track.Loaded() && cars.size() > 0 && frame_id % frames_to_record == 0 && frame_id >= 10) {
			if (dataset_render_mode == RENDER_MODE_POSE) {
				sprintf(filename, "%s\\%03d_%06d.txt", dataset_folder.c_str(), sequence_id, image_id++);
				std::ofstream out(filename);
				MATHVECTOR<float, 3U> position = cars.front().GetCameras().front()->GetPosition();

				float x, y, z;
				QUATERNION<float> q = cars.front().GetCameras().front()->GetOrientation();
				q.GetEulerZYX(x, y, z);
			
				out << position[0] << " " << position[1] << " " << position[2] << " " << x << " " << y << " " << z;
				out.close();
			}
			else {
				sprintf(filename, "%s\\%03d_%06d.png", dataset_folder.c_str(), sequence_id, image_id++);
				window.Screenshot(std::string(filename));
			}
		}
		...
		if (should_record && !gui.Active() && cars.size() > 0 && track.Loaded())
			++frame_id; 
...
void GAME::AdvanceGameLogic()
{
	...
	if (track.Loaded() && !pause && !gui.Active())
	{
		...
		{
			if (carcontrols_local.second.inputs[CARINPUT::REPLAY_FF] > 0.000000001) {
			...
			if (carcontrols_local.second.inputs[CARINPUT::REPLAY_FF] > 0.000000001) {
				printf("\nStart recording\n\n");
				replay.StartRecording(settings.GetTrack(), error_output, cars, settings.GetNumberOfLaps());
				replay.RecordFrame(cars);
			}
		}
...
void GAME::UpdateCarInputs(CAR & car)
{
	...
	if (timer.Staging() || ((int)timer.GetCurrentLap(cartimerids[&car]) > race_laps && race_laps > 0) || car.GetCarId() >= 1000)
	{
	...
	{
		if (start_camera_pose && !replay.GetPlaying())
			active_camera->Reset(start_camera_pos, start_camera_orient);
		else
	...
	if (carcontrol.GetInput(CARINPUT::REPLAY_RW)) {
		float xx, yy, zz;
		QUATERNION<float> crot = active_camera->GetOrientation();
		crot.GetEulerZYX(xx, yy, zz);
				
		active_camera->Rotate(-xx, 0);
	}
	...
	float dy = TickPeriod() * carcontrol.GetInput(CARINPUT::ZOOM_IN);
	float strafe = TickPeriod() *  carcontrol.GetInput(CARINPUT::ZOOM_OUT);
	
	MATHVECTOR<float, 3> zoom(direction::Forward * 6 * dy);
	active_camera->Rotate(0.2 * up, 0.2 * left);
	active_camera->Move(zoom[0], zoom[1], zoom[2]);

	active_camera->Rotate(0, -1.57079632679489661923132);
	active_camera->Move(0, -2 * strafe, 0);
	active_camera->Rotate(0, 1.57079632679489661923132);
...
bool GAME::NewGame(bool playreplay, bool addopponents, int num_laps)
{
	...
	if (cars_list.size() > 0) {
		...
		for (int i=0; i<cars_list.size(); ++i) {
			LoadCar(cars_list[i].name, "default", cars_list[i].color,
					cars_list[i].position, cars_list[i].orientation,
					false, false, "", 1000+i);
		}
	}
...
std::string GAME::GetReplayRecordingFilename()
{
	...
	// Time string.
	char timestr[]= "MM-DD-hh-mm-ss";
	const char format[] = "%m-%d-%H-%M-%S";
	...
	const CARID car_id)
{
	...
	if (carfile.empty())
	{
		...
		file_open_basic fopen(pathmanager.GetCarPath(car_name), car_parts_path);
	...
	CAR & car = cars.back();
	if (!car.LoadGraphics(
		carconf, car_parts_path, car_dir, car_name,
