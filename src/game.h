...
private:
	...
	std::string carlist_filename;
	std::vector<CarDescription> cars_list;

	bool					start_camera_pose;
	MATHVECTOR<float,3>		start_camera_pos;
	QUATERNION<float>		start_camera_orient;

	bool					should_record;
	DATASET_RENDER_MODE		dataset_render_mode;
