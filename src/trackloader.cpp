...
TRACK::LOADER::LOADER(
	...
	const DATASET_RENDER_MODE mode) :
	content(content),
	world(world),
	data(data),
	...
	dataset_render_mode(mode)
{
	...
	if (dataset_render_mode == RENDER_MODE_DEPTH ||
		...
		objectpath = objectpath + "/segmentation";
		objectdir = objectdir + "/segmentation";
	}
}
...
std::pair<bool, bool> TRACK::LOADER::ContinueOld()
{
	...
	if (dataset_render_mode == RENDER_MODE_SEGMENTATION)
		object.mipmap = false;
