...
bool REPLAY::StartPlaying(const std::string & replayfilename, std::ostream & error_output)
{
	...
	for (int i=0; i<carcolors.size(); ++i) {
		MATHVECTOR<float, 3> hsv_color, rgb_color;
		rgb_color = carcolors[i];
		RGBtoHSV(rgb_color[0], rgb_color[1], rgb_color[2], hsv_color[0], hsv_color[1], hsv_color[2]);
		carcolors[i] = hsv_color;
	}
